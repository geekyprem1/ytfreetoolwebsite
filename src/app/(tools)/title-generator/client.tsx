...
export function TitleGeneratorClient() {
  ...
  const handleGenerate = () => {
    execute('/api/ai/generate-titles', {
      ...
      body: JSON.stringify({ topic, keyword, language, tone: tone as never, count }),
    });
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        ...
        <ToolInput label="Number of Titles">
          <Slider value={count} min={5} max={20} step={5} onChange={setCount} disabled={isLoading} />
        </ToolInput>
      </div>

      <Button onClick={handleGenerate} disabled={!isValid || isLoading} className="w-full">
        {isLoading ? 'Generating...' : `Generate ${count} Titles`}
      </Button>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between mb-3">
              <p className="text-xs text-muted-foreground">
                Tokens used: {data.meta.tokensUsed} · Model: {data.meta.model}
              </p>
              <OutputActions
                copyText={data.titles.map((t) => t.title).join('\n')}
